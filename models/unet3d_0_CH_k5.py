...
class UNet(nn.Module):
    ...
    def _block(self, in_channels, mid_channels, out_channels, kernel_size=(5, 5, 5), dtype=torch.float32):
        '''
        Building unit of our encoder and decoder architecture. Takes the channels from the previous block and doubles the channels in the in the output feature map
        '''
    ...
    def final_block(self, in_channels, mid_channels, out_channels, kernel_size=(5, 5, 5), dtype=torch.float32):
        return torch.nn.Sequential(
            Conv3dCH(
                in_channels=in_channels,
                out_channels=mid_channels,
    ...
    def __init__(self, n_classes: int = 1):
        ...
        # Decode
        self.upconv3 = ConvTranspose3dCH(in_channels=init_feat * 16, out_channels=init_feat * 16, kernel_size=(5, 5, 5), stride=2, padding=2, output_padding=1, bias=False)
        self.conv_decoder3 = self._block(init_feat * 16 + init_feat * 8, init_feat * 8, init_feat * 8)
        self.upconv2 = ConvTranspose3dCH(in_channels=init_feat * 8, out_channels=init_feat * 8, kernel_size=(5, 5, 5), stride=2, padding=2, output_padding=1, bias=False)
        self.conv_decoder2 = self._block(init_feat * 8 + init_feat * 4, init_feat * 4, init_feat * 4)
        self.upconv1 = ConvTranspose3dCH(in_channels=init_feat * 4, out_channels=init_feat * 4, kernel_size=(5, 5, 5), stride=2, padding=2, output_padding=1, bias=False)
