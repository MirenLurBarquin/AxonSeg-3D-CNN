...
from torchio.data.sampler.weighted import WeightedSampler
...
class CustomGridSampler(PatchSampler):
    ...
    For training with augmentation (includes Weighted Sampler).
    """
    ...
    def _compute_locations(self, subject: Subject):
        ...
        return self.generate_patch_corners(*sizes, subject=subject)
    ...
    def _generate_patches(
        ...
        sizes = subject.spatial_shape, self.patch_size, self.patch_overlap
        self._parse_sizes(*sizes)
        locations = self.generate_patch_corners(imsize=sizes[0], patchsize=sizes[1], patch_overlap=sizes[2], subject=subject)
        if len(locations) != 0:
            patches_left = num_patches if num_patches is not None else True
            while patches_left:
                rand_loc = np.random.randint(low=0, high=locations.shape[0])
                index_ini = locations[rand_loc][:3]
                yield self.extract_patch(subject, index_ini)
                if num_patches is not None:
                    patches_left -= 1

        else:
            sampler = WeightedSampler(self.patchsize, probability_map=self.mask_name)
            probability_map = sampler.get_probability_map(subject)
            probability_map_array = sampler.process_probability_map(
                probability_map,
                subject,
            )
            cdf = sampler.get_cumulative_distribution_function(probability_map_array)

            patches_left = num_patches if num_patches is not None else True
            while patches_left:
                yield sampler.extract_patch(subject, probability_map_array, cdf)
                if num_patches is not None:
                    patches_left -= 1
    ...
    def generate_patch_corners(self, imsize, patchsize, patch_overlap, subject: Subject):
        ...
        for stride_perm in stride_perms:
            Y, X = np.ogrid[:imsize[1] + 1, :imsize[0] + 1]

            legal_patches = np.zeros(imsize)  # Every pixel in the image could be a possible down-left corner for a patch
            x_range = list(range(0, imsize[0] - patchsize[0], stride_perm[0]))  # - patchsize bc. not to use corners that do not fit a whole patch on the left of the img
