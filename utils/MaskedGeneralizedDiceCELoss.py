...
class MaskedGeneralizedDiceCELoss(_Loss):
    '''
    Combines the Masked Generalized Dice Loss and the CE Loss
    '''
    ...
    def __init__(
        ...
        self.masked_dice = MaskedGeneralizedDiceLoss(
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        masked_dice_loss = self.masked_dice(input, target, mask)
        ce_loss = self.ce(input, target)
        total_loss: torch.Tensor = self.lambda_dice * masked_dice_loss + self.lambda_ce * ce_loss
